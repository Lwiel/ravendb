...
namespace Raven.Server.Documents.ETL
{
    public sealed class EtlLoader : IDisposable, ITombstoneAware
    {
        ...
        {
            var newOpenAiDestinations = new List<AiEtlConfiguration>() { new AiEtlConfiguration() { ConnectionStringName = "ConnectionStringName", Name = "ConnectionName", Transforms = new List<Transformation>() { new Transformation() { Collections = new List<string>() { "Test" }, Name = "CoolName", Script = "loadToWhatever(){}" } }, FieldsToInclude = new List<string>() { "Name" } } };
            ...
            List<string> explanations)
        {
            lock (_loadProcessedLock)
            {
                ...
                if (newOpenAiDestinations != null && newOpenAiDestinations.Count > 0)
                    newProcesses.AddRange(GetRelevantProcesses<AiEtlConfiguration, AiEtlConnectionString>(newOpenAiDestinations, ensureUniqueConfigurationNames));
                ...
                switch (config.EtlType)
                {
                    ...
                    case EtlType.Ai:
                        ...
                        if (_databaseRecord.OpenAiConnectionStrings.TryGetValue(config.ConnectionStringName, out var openAiConnection))
                            ...
                            connectionStringNotFound = true;
                        */
                        var openAiConnection = new AiEtlConnectionString();
        ...
        public void HandleDatabaseRecordChange(DatabaseRecord record)
        {
            ...
            {
                if (IsMyEtlTask<AiEtlConfiguration, AiEtlConnectionString>(record, config, ref responsibleNodes, out explanations))
                {
                    myAiEtl.Add(config);
                }
            }
