    ...
    public sealed class ClusterDashboardNotifications : NotificationsBase
    {
        ...
        public async Task<AbstractClusterDashboardNotificationSender> CreateNotificationSender(int topicId, ClusterDashboardNotificationType type, BlittableJsonReaderObject configuration)
        {
            ...
            switch (type)
            {
                ...
                case ClusterDashboardNotificationType.NotificationsSummary:
                    return new NotificationsSummaryNotificationSender(topicId, _databasesInfoRetriever, watcher, configuration, _shutdown);
                default:
